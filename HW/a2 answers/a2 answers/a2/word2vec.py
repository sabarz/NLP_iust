...
def negSamplingLossAndGradient(
    ...
    v_c = outsideVectors[negSampleWordIndices]  
    hold = v_c @ centerWordVec

    loss = -np.log(sigmoid(u_o @ centerWordVec)) - np.sum(np.log(1 - sigmoid(hold)))
    gradCenterVec = -(1 - sigmoid(u_o @ centerWordVec)) * u_o + np.sum(sigmoid(hold).reshape(-1, 1) * v_c , axis = 0)

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = -(1 - sigmoid(u_o @ centerWordVec)) * centerWordVec
    
    for i, index in enumerate(negSampleWordIndices):
        gradOutsideVecs[index] += sigmoid(hold)[i] * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        index = word2Ind[outsideWord]
        l1, l2, l3 = word2vecLossAndGradient(centerWordVector, index, outsideVectors, dataset)
        loss += l1
        gradCenterVecs[word2Ind[currentCenterWord]] += l2
        gradOutsideVectors += l3
