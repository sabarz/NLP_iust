...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        ###         - Note that the shape of th
        # e tensor output returned by the encoder RNN is (src_len, b, h*2) and we want to
        ...
        hold , (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens = pad_packed_sequence(hold)[0].permute(1, 0, 2)

        #4
        concated_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim = 1)
        init_decoder_hidden = self.h_projection(concated_hidden)
        concated_cell = torch.cat((last_cell[0], last_cell[1]), dim = 1)
        init_decoder_cell = self.c_projection(concated_cell)
