...
if args.function == 'pretrain':
    ...
    trainer = trainer.Trainer(model, pretrain_dataset, None, t)
    trainer.train()
    ...
    hold = open(args.finetune_corpus_path, encoding='utf-8').read()
    TrainDataset = dataset.NameDataset(pretrain_dataset, hold)

    if args.eval_corpus_path is not None:
        EvalCorpus = open(args.eval_corpus_path).read()
        EvalDataset = dataset.NameDataset(pretrain_dataset, EvalCorpus)
    else:
        EvalDataset = None
    
    trainer = trainer.Trainer(model, TrainDataset, EvalDataset, t)
    trainer.train()
