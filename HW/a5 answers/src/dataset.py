...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1
        l = round(len(document) / 4)
        masklen = l + random.randint(-l, l)
        index = random.randint(0, masklen)

        # 2
        prefix = document[ : index]
        suffix = document[index + masklen:]
        masked_content = document[index : index + masklen]
